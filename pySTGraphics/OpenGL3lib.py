...
class GL_BatchImageRenderer():
    ...
    def __init__(self, nrlayers):
        logging.debug("*************************************")
        logging.debug("* Initialise batch image renderer ")
        logging.debug("*************************************")
    ...
    def initialise (self):
        self.layersContent =[]
        self.layerBufferIDs = []
        self.layersVertexData = [self.nrlayers]
        self.initShaders()
        self.program = buildProgram(self.vertex_code,self.fragment_code)
        
        for i in range (0, self.nrlayers):
    ...
    def addImageToRenderQueue(self, GL_Image, layer = 0):
        ...
        logging.debug("adding image %s to render queue on layer %s", GL_Image.name ,layer)
    ...
    def fillBuffers(self):
        logging.debug("GL_BatchImageRenderer: fillBuffers")
...
class GL_Image:
    ...
    def __init__(self, GLtexture,cliprect = None,cliprect_origin = None):
        ...
        logging.debug("*************************************")
        logging.debug("* Initialise image %s, ID: %s", GLtexture.name, self.id)
        logging.debug("*************************************")
        ...
        if cliprect !=None:
            ...
            self.width = cliprect[0]
            self.height = cliprect[1]
...
class GL_rectangle:
    ...
    def __init__(self, width, height, linewidth = 1, color = COLOR_GREEN):
        ...
        self.program = buildProgram(self.vertex_code,self.fragment_code)
        logging.debug("Init GL_Rectangle, width: %s, height: %s", self.width,self.height)
...
class GL_Image_OLD:
    ...
    def __init__(self, GLtexture,cliprect = None,origin = None):
        logging.debug("*************************************")
        logging.debug("* Initialise image %s", GLtexture.name)
        logging.debug("*************************************")
        self.name = GLtexture.name
        self.texture = GLtexture.texture
        self.width = GLtexture.width
